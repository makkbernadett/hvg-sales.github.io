...
function IsOrderPossible(isItAsync, callback) {
    var data = '{ "productWebId": "AZ_KW_IRATMINTA"}';
        ...
        success: function (msg) {
            if (!callback) {
                callback = function (ret) {
                    if (ret === false) {
                        alert("Önnek már van megrendelése a GDPR – Kereskedelmi iratmintacsomagra. Hozzáférési problémák esetén keresse ügyfélszolgálatunkat munkanapokon 9:00-től 14:00 óráig, az ugyfelszolgalat@hvg.hu email címen vagy a (+36-1)-436 2045 telefonszámon.");
                    }
                };
            }
            ret = callback(msg.d);
