...
(function($) {
	...
	$.Juitter = {
					...
					$.each(json.results,function(i,item) {
						if(e==1 || (i<numMSG && item.id>ultID)){
							...
							if (item.text != "undefined") {
								...
								if(fromID=="image") mHTML="<div class='twtr-avatar'><a href='http://www.twitter.com/"+item.from_user+"'><img src='"+item.profile_image_url+"' alt='"+item.from_user+"' class='juitterAvatar' /></a></div><div class='twtr-tweet-text'>"+$.Juitter.textFormat(tweet)+" <br/> <a href='" + link + "' class='JRM' "+openLink+"><span class='time'>"+$.Juitter.parseTime(item.created_at)+"</span></a></div>";
		...
		parseTime: function(h)
		{
		    ...
		    if(i<c*7){return"éppen most"}
		    if(i<d){return Math.floor(i/c)+" másodperce"}
		    if(i<d*2){return"egy perce"}
		    if(i<e){return Math.floor(i/d)+" perce"}
		    if(i<e*2){return"egy órája"}
		    if(i<g){return Math.floor(i/e)+" órája"}
		    if(i>g&&i<g*2){return"yesterday"}
		    if(i<g*365){return Math.floor(i/g)+" napja"}
		    else{return"több mint egy éve"}
        }
	};	
})(jQuery);